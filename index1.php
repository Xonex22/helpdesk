...
<!--<style>
        body {
            ...
            width: 125%;  /* Adjust width to counteract the scale */
            height: 125%; /* Adjust height to counteract the scale */
        }
    </style>-->
    <!-- ./webpage size -->
    <!-- scroll -->
...
<body id="page-top">
 ...
 <script>
        // Function to fetch and update content from the server
        function updateContent() {
            fetch('./tools/data-list.php') // Replace 'load_content.php' with your server-side script URL
                .then(response => response.text()) // Parse the response as text
                .then(data => {
                    // Update the content inside the div with the new data
                    document.getElementById('content-container').innerHTML = data;
                })
                .catch(error => console.error('Error fetching content:', error)); // Handle errors
        }

        // Automatically reload content every 5 seconds (5000 milliseconds)
        setInterval(updateContent, 5000);

        // Load content immediately after the page loads
